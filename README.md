...
контейнера, поэтому первый старт сразу поднимает установщик — без ожидания
серверов Apple.
...
Если сеть сборочного агента не пускают на `osrecovery.apple.com`, собирай с
`--build-arg BAKE_RECOVERY=N` — тогда recovery скачается при первом старте
контейнера, как в оригинальном `dockurr/macos`.
